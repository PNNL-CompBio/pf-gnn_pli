...
from gnnp_dataset_multigpu import MolDataset, collate_fn, DTISampler
import sys

now = time.localtime()
time_hold = "%04d-%02d-%02d %02d:%02d:%02d" % (now.tm_year, now.tm_mon, now.tm_mday, now.tm_hour, now.tm_min, now.tm_sec)
print(time_hold)
...
parser.add_argument("--train_keys", help="train keys", type=str, default='keys/2k_rand_79_dude_pdbbind_numpy_train.pkl')
#parser.add_argument("--valid_keys", help="valid keys", type=str, default='keys/pdbbind_dude_small_valid.pkl')
parser.add_argument("--test_keys", help="test keys", type=str, default='keys/2k_rand_79_dude_pdbbind_numpy_test.pkl')
...
#Pull keys. Each key contains a pickle set of the ligand and protein molecule files
...
#Provides option to include validation split as well
...
num_train_neg = [len([0 for k in x if 'ZINC' in k or 'neg' in k]) for x in train_keys] #was ZINCID for some reason
...
def train_DDP(rank, world_size,):
    ...
    for epoch in range(args.epoch):
        ...
        #initialize load time, train time, optimizer time
        ...
        # save each gpu log in an individual csv
        if epoch == 0:
